...
export default function ScanPage() {
  ...
  const [currentPage, setCurrentPage] = useState<'scan' | 'history' | 'floormap' | 'schedules'>('scan')
  const [itemsToShow, setItemsToShow] = useState(5)
  const [selectedMonth, setSelectedMonth] = useState<string>('all')
  const [currentPaginationPage, setCurrentPaginationPage] = useState(1)
  const [selectedFloor, setSelectedFloor] = useState('1')
  const [floorPaginationPage, setFloorPaginationPage] = useState(1)
  const [desksPerPage, setDesksPerPage] = useState(12)
  ...
  const processQRCode = (qrCode: string) => {
    ...
    if (!trimmedCode) {
      ...
      // Don't show error for empty codes - just continue scanning
      setIsScanning(true)
      return
    }
    ...
    if (!cleanDeskId) {
      ...
      // Don't show error for invalid codes - just continue scanning
      setIsScanning(true)
      return
    }
